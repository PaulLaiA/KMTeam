...
**Previous** :: [[Study_dPaul/Golang/Go 基礎教學/1-3-1|数组 Array]]
...
上一节介绍的[[Study_dPaul/Golang/Go 基礎教學/1-3-1|数组]]在 Go 语言中没那么常用，更常用的数据结构是切片，即**动态数组**，其长度并不固定，我们可以向切片中追加元素，它会在容量不足时自动扩容。
...
编译期间的切片是 [`cmd/compile/internal/types.Slice`](https://draveness.me/golang/tree/cmd/compile/internal/types.Slice) 类型的，
但是在运行时切片可以由如下的 [`reflect.SliceHeader`](https://draveness.me/golang/tree/reflect.SliceHeader) 结构体表示，其中:
-  `Data` 是指向数组的指针;
-  `Len` 是当前切片的长度；
...
数组中的元素只是逻辑上的概念，
底层存储其实都是连续的，
所以我们可以将切片理解成一片连续的内存空间加上长度与容量的标识。
![[Study_dPaul/Golang/Go 基礎教學/_annex/Pasted image 20211103160553.png]]
...
![[Study_dPaul/Golang/Go 基礎教學/_annex/Pasted image 20211103160610.png]]
...
![[Study_dPaul/Golang/Go 基礎教學/_annex/Pasted image 20211103160633.png]]
